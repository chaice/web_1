...
public class ReadExcelUtil {
    ...
    public static Map<String, StoreInfo> readStoreInfoFromXLSX(String filePath) {
        ...
        try {
            ...
            Sheet sheet = workbook2007.getSheet("0");
            // 开始循环遍历行，表头不处理，从1开始
...
//                    XSSFCell cell = row.getCell(j);// 获取单元格对象
                    ...
                    if (j == 0) {
                        ...
                        storeInfo.setNetmask("255.255.255.192");
                    } else if (j == 1) {
                        storeInfo.setSubnet(cellStr);
                    } else if (j == 2) {
                        storeInfo.setGateway(cellStr);
                    } else if (j == 3) {
                        storeInfo.setVlan10("有".equals(cellStr) ? true : false);
                    } else if (j == 4) {
                        storeInfo.setVlan20("有".equals(cellStr) ? true : false);
                    }
                }
